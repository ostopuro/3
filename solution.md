...
- **Amenities**：字符串去空格并转小写去重，保留小写格式, 按照字母表排序.
  - 去重逻辑, 需要细化到业务维度, 有一些特殊规则, 请抽成单独的文件, 应用策略模式.
    比如:
    - 当出现 indoor pool 或者 outdoor pool 的时候, 就统一额外加上 pool
    - BusinessCenter 和 business center 是重复的, 统一使用后者
...
- 增加一个 webUI, 可以查看 store 的内容, 方便调试用, 有列表总览和针对 id 进行单独查询展示的实用功能, 极简,够用就行, 非必要不要引入过多依赖.
